...
class Config:
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        ...
        self.default_config = {
            'server_url': 'http://172.24.194.92:8000',
            'server_host': '172.24.194.92',  # TCP 서버 호스트
    ...
    def _load_config(self) -> dict:
            ...
            try:
                with open(self.root_config_file, 'r', encoding='utf-8') as f:
                    ...
                    if 'server' in root_config:
                        ...
                        config['server_url'] = f"http://{server.get('host', '172.24.194.92')}:{server.get('http_port', 8000)}"
                    # 에이전트 설정 병합
        ...
        # 2. 로컬 config.json에서 설정 읽기 (우선순위 높음)
    ...
    def server_url(self) -> str:
        return self.get('server_url', 'http://172.24.194.92:8000')
    ...
    def server_host(self) -> str:
        return self.get('server_host', '172.24.194.92')
